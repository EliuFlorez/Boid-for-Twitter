...
public class NetworkedCacheableImageView extends CacheableImageView {
        ...
        protected CacheableBitmapDrawable doInBackground(String... params) {
            try {
                ...
                if (null == result) {
                    ...
                    // Add to cache
                    if (isRounded) {
                        Bitmap bitmap = Utilities.getRoundedImage(BitmapFactory.decodeStream(is, null, mDecodeOpts), 90F);
                        result = mCache.put(url, bitmap);
                    } else {
    ...
    public NetworkedCacheableImageView(Context context) {
        super(context);
        mCache = BoidApplication.get(context).getBitmapCache();
    }
    ...
    public boolean loadImage(String url, final boolean fullSize) {
        ...
        if (null != wrapper) {
            ...
            mCurrentTask.isRounded = this.isRounded;
